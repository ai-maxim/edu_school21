/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   tr_intersect.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: qdong <qdong@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/13 11:34:48 by qdong             #+#    #+#             */
/*   Updated: 2021/04/14 19:31:47 by qdong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

t_close	tr_intersect(t_vec *orig, t_vec *dir, void *data, double lim[2])
{
	t_perem	p;
	t_tr	*tr;
	t_close	cl;

	tr = data;
	init_zero(&cl);
	p.v1 = substract_vec(tr->c2, tr->c1);
	p.v2 = substract_vec(tr->c3, tr->c1);
	p.vec = cross_product(*dir, p.v2);
	p.det = dpv(p.v1, p.vec);
	if (fabs(p.det) < 0.00001)
		return (cl);
	p.c = 1.0 / p.det;
	p.tvec = substract_vec(*orig, tr->c1);
	p.a = dpv(p.tvec, p.vec) * p.c;
	if (p.a < 0 || p.a > 1)
		return (cl);
	p.tvec = cross_product(p.tvec, p.v1);
	p.b = dpv(*dir, p.tvec) * p.c;
	if (p.b < 0 || p.a + p.b > 1)
		return (cl);
	p.t = dpv(p.v2, p.tvec) * p.c;
	if (p.t < lim[1] && p.t > lim[0])
	{
		cl.t = p.t;
		if (lim[1] == MAX_DIST)
		{
			cl.obj_col = tr->color;
			cl.dot_inter = multip_vn(*dir, p.t);
			cl.dot_inter = addit_vec(cl.dot_inter, *orig);
			cl.normal = cross_product(p.v1, p.v2);
			cl.normal = normalize_vec(cl.normal);
			invert_normal(&cl.normal, cl.dot_inter, orig);
		}
	}
	return (cl);
}
